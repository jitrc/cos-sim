...
import ru.cos.nissan.information.TreeItem;
...
public class CarInformationTempHandler {
	...
	public void refreshTraceCarInfo(VehicleDTO vdto)
	{
		...
		this.model.updateCarInformation(root, linkLocationData.getLinkId(),
				linkLocationData.getSegmentId(),vdto.getSpeed(), linkLocationData.getLaneIndex(),vdto.getAgentId());
		if (universeExpanded) table.expandPath(universe);
		if (carExpanded) {
			table.expandRow(carRow);
		}
	...
	private int getIsCarLoad(TreePath root,boolean univExpanded)
	{
		...
		//TreePath carPath = table.getPathForRow(list.size()+1);
		return list.size()+1;
