package ru.cos.sim.visualizer.font;
 ...
 * 
 * @author Ivan Dudinov - modifications according 
 * current project
 */
public class TrueTypeFont {
	...
	private void createSet( char[] customCharsArray ) {
		...
		try {
			...
			fontTexture = TexturesManager.getInstance().getLoader()
	...
	public void drawString(float x, float y, String whatchars,
			ru.cos.sim.visualizer.color.Color color) {
		drawString(x,y,whatchars,color,0,whatchars.length()-1);
	}
	
	private void adjustBounds(float stringWidth){
		if (!bounded) {
			scaleY = 1;
			scaleX = 1;
			return;
		...
		scaleX = boundWidth/stringWidth;
		scaleY = boundheight/stringWidth;
	}
	
	private void begin() {
		GL11.glPushMatrix();
		switch (align) {
		case Center: GL11.glTranslatef(-boundWidth/2.0f, (boundheight/2.0f) + 0.2f, 0);
	 ...
	 */
	public void drawString(float x, float y, String whatchars,
			ru.cos.sim.visualizer.color.Color color, int startIndex, int endIndex) {
		
		int stringWidth = getWidth(whatchars);
		adjustBounds(stringWidth);
			...
			if( intObject != null ) {
				if ((i >= startIndex) || (i <= endIndex)) {
					drawQuad((x + totalwidth), y,
							(x + totalwidth + intObject.width*scaleX),
							(y + intObject.height*scaleY), intObject.storedX,
							intObject.storedY, intObject.storedX + intObject.width,
							intObject.storedY + intObject.height);
				}
				totalwidth += intObject.width*scaleX + 0.1f;
			}
		}
	 ...
	 */
	public void drawString(float x, float y, String whatchars) {
		drawString(x, y, whatchars, ru.cos.sim.visualizer.color.Color.white);
	}
