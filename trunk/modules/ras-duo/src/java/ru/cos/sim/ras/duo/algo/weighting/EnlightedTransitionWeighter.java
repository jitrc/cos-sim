...
public class EnlightedTransitionWeighter implements Weighter {
	...
	public EnlightedTransitionWeighter(TransitionEdge transition, TravelTimeProvider travelTimeProvider) {
		...
		if (transition.getLightsNetworkData() instanceof RegularTLNData)
			initialize((RegularTLNData)transition.getLightsNetworkData());
	}
	...
	private float getAwaitTimeAt(float time) {
		// If there's no phases, all-green
		if (phases.size() == 0) return 0;
		
		// If there's no green at all
		...
		if (phases.get(i).isGreen()) {
			...
			PhaseInfo phase;
			while (!(phase = phases.get(i = (i + 1) % phases.size())).isGreen())
				time -= phase.getDuration();
			return -time;
		}
	...
	public float getWeight(PathExtensions extensions) {
		// Not initialized lights table means either an all-green lights table or not regular lights type   
		if (!isInitialized()) 
			return 0;
