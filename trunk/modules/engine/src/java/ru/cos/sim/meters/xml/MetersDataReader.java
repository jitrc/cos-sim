...
public class MetersDataReader {
	...
	public static Set<MeterData> read(Element metersElement) {
		...
		for (Object meterObj:metersElement.getChildren()){
			...
			case NetworkAverageTravelSpeedMeter:{
				meterData = new MeterData(); // no any extra props
				break;
			}
			case LinkAverageTravelSpeedMeter:{
				int linkId = Integer.parseInt(meterElement.getChildText(LINK_ID, NS));
				LinkAverageTravelSpeedMeterInitData linkAverageTravelSpeedMeterInitData =
					new LinkAverageTravelSpeedMeterInitData();
			...
			default: throw new SimulationEngineRuntimeException("Unknown type of meter: "+meterType);
			}
	...
	private static ModesInitData initMeterModes(Element dataCollectionModes) {
		...
		if (dataCollectionModes==null) return modesInitData; // in case of absence modes element
		// average data mode
		if (dataCollectionModes.getChild(AVERAGE_DATA_MODE, NS)!=null)
			modesInitData.setAverageDataMeasured(true);
		// history mode
		...
		// average data on scheduled periods mode
		...
		if (scheduledAverageDataMode!=null){
			List<TimePeriod> schedule = 
				initMeterSchedule(scheduledAverageDataMode.getChild(SCHEDULE, NS));
			modesInitData.setAverageDataCollectorSchedule(schedule);
		}
		// history on scheduled periods mode
		...
		if (scheduledHistoryDataMode!=null){
			List<TimePeriod> schedule = 
				initMeterSchedule(scheduledHistoryDataMode.getChild(SCHEDULE, NS));
	...
	private static List<TimePeriod> initMeterSchedule(Element scheduleElement) {
		...
		for (Element timePeriodElement:timePeroiodElements){
			float timeFrom =
				Float.parseFloat(timePeriodElement.getChildText(FROM, NS));
			float timeTo =
				Float.parseFloat(timePeriodElement.getChildText(TO,NS));
